...
void ClapTrap:: takeDamage(unsigned int amount) {
	...
	{
		std:: cout <<"Sorry! ClapTrap target name:- " <<this->_name
		<<" cannot attack because you have no energyPoints or hitPoints "
		<<std:: endl;
		return ;
	}
...
void ClapTrap:: beRepaired(unsigned int amount) {
	...
	else
	{
		...
		this->_energyPoint -= 1;
		this->_hitPoint += amount;
		std:: cout<< "ClapTrap " <<this->_name <<"'s available hitPoint after repaired: " <<this->_hitPoint <<std::endl;
		std:: cout<< "ClapTrap " <<this->_name <<"'s available energyPoints after repaired: " <<this->_energyPoint <<std::endl;
